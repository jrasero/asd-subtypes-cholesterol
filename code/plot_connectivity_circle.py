...
#    if len(node_heights)>1:
    height = np.ones(n_nodes)*np.array(node_heights)
 #   else:
   #     height = np.ones(n_nodes) * 1.0
    ...
    for name, angle_rad, angle_deg in zip(node_names, node_angles, angles_deg):
        ...
        axes.text(angle_rad, 10.4, name, size=fontsize_names, weight = fontweight_names,
    ...
    if colorbar:
        ...
        cb = plt.colorbar(sm, ax=axes, use_gridspec=False,
                          ...
                          anchor=colorbar_pos, orientation='horizontal')
