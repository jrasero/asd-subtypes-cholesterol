function res_cluster = cluster_consensus(CC, dist_type, kind, nshuff, gamma, seed)
    ...
    switch dist_type
        case 'pearson'
            for ii=1:n_rois
                % DOing this would be more correct, but gives the same
                %temp =  squeeze(CC(:, ii,:));
                %temp(:, ii) = [];
                %D(:, :, ii) = sqrt(2*squareform(pdist(temp, 'correlation')));
                D(:, :, ii) = sqrt(2*squareform(pdist(squeeze(CC(:, ii,:)), 'correlation')));
            end 
        case 'spearman'
            for ii=1:n_rois
                D(:, :, ii) = sqrt(2*squareform(pdist(squeeze(CC(:, ii,:)), dist_type)));
            end 
        otherwise 
            for ii=1:n_rois
                D(:, :, ii) = squareform(pdist(squeeze(CC(:, ii,:)), dist_type));
            end 
    end
    ...
    res_cluster.Q = Q;
    res_cluster.S = S;
    %res_cluster.clus_subj = clus_subj;
    
end

%save(strcat(workingDir, "/data/consMat_asd_denoised_kul_fisher_OLS.mat"), 'con', 'S', 'Q', 'clus_subj')
%save(strcat(workingDir, "/data/consMat_asd_combat.mat"), 'B', 'P','C', 'S', 'Q', 'clus_subj')

%save(strcat("/media/javier/data/amaia-abide/", "/data/consMat_asd_denoised_kul_fisher_OLS.mat"), 'con', 'S', 'Q', 'clus_subj')